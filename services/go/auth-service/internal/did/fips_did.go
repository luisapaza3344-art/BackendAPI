...
func (d *FIPSDIDService) generateFIPSKeyPair(keyType string) (interface{}, interface{}, error) {
        // FIPS 140-3 Level 3 enforcement: Only allow FIPS-approved algorithms when FIPS mode is enabled
        if d.fipsMode {
                if err := d.validateFIPSKeyType(keyType); err != nil {
                        return nil, nil, fmt.Errorf("FIPS key type validation failed: %w", err)
                }
        }
        ...
        case "Ed25519":
                if d.fipsMode {
                        return nil, nil, fmt.Errorf("Ed25519 is not FIPS 140-3 Level 3 approved. Use P-256, P-384, P-521, RSA-2048, RSA-3072, or RSA-4096")
                }
                // Generate Ed25519 key pair (non-FIPS mode only)
        ...
        case "P-384":
                // Generate FIPS-approved ECDSA P-384 key pair
                privateKey, err := ecdsa.GenerateKey(elliptic.P384(), rand.Reader)
                if err != nil {
                        return nil, nil, fmt.Errorf("ECDSA P-384 key generation failed: %w", err)
                }
                return privateKey, &privateKey.PublicKey, nil

        case "P-521":
                // Generate FIPS-approved ECDSA P-521 key pair
                privateKey, err := ecdsa.GenerateKey(elliptic.P521(), rand.Reader)
                if err != nil {
                        return nil, nil, fmt.Errorf("ECDSA P-521 key generation failed: %w", err)
                }
                return privateKey, &privateKey.PublicKey, nil

        default:
                return nil, nil, fmt.Errorf("unsupported key type: %s. FIPS mode requires: P-256, P-384, P-521, RSA-2048, RSA-3072, RSA-4096", keyType)
        }
}

// validateFIPSKeyType ensures only FIPS-approved key types are used
func (d *FIPSDIDService) validateFIPSKeyType(keyType string) error {
        ...
        if !approvedTypes[keyType] {
                d.logger.DIDLog("fips_validation", "", keyType, "error", map[string]interface{}{
                        "error": fmt.Sprintf("Key type '%s' is not FIPS 140-3 Level 3 approved", keyType),
                        "approved_types": "P-256, P-384, P-521, RSA-2048, RSA-3072, RSA-4096",
                })
...
func (d *FIPSDIDService) publicKeyToJWK(publicKey interface{}, keyType string) (*JWK, error) {
        // FIPS 140-3 Level 3 enforcement: Validate key type before processing
        if d.fipsMode {
                if err := d.validateFIPSKeyType(keyType); err != nil {
                        return nil, fmt.Errorf("FIPS key type validation failed: %w", err)
                }
        }
        ...
        case "Ed25519":
                if d.fipsMode {
                        return nil, fmt.Errorf("Ed25519 is not FIPS 140-3 Level 3 approved")
                }
        ...
        return nil, fmt.Errorf("unsupported key type '%s' or invalid public key. FIPS mode requires: P-256, P-384, P-521, RSA-2048, RSA-3072, RSA-4096", keyType)
}
...
func (d *FIPSDIDService) getVerificationMethodType(keyType string) string {
        ...
        case "Ed25519":
                if d.fipsMode {
                        // Should not reach here due to earlier validation, but provide safety
                        return "FIPS_VIOLATION_Ed25519_NOT_APPROVED"
                }
